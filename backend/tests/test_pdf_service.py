...
class TestPDFService(unittest.TestCase):
    ...
    def test_process_pdf_success(self, mock_prepare_text, mock_extract_text, mock_update_job):
        ...
        mock_prepare_text.return_value = "Prepared text from PDF"
        
        # Call the method
        result = self.pdf_service.process_pdf("test.pdf", "job123")
        
        # Assertions
        self.assertEqual(result, "Prepared text from PDF")
        mock_extract_text.assert_called_once_with("test.pdf", "job123")
        ...
        mock_update_job.assert_any_call("job123", {
            "status": "completed", 
            "progress": 100,
            "result": "Prepared text from PDF"
        })
